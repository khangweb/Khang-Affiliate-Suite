    ...
    <script type="module">
        ...
        app.component('v-product-variation-item', {
            ...
            methods: {
                ...
                setFiles() {
                    if (!Array.isArray(this.variant.images)) {
                        this.variant.images = [];
                    }
                    ...
                    this.variant.images.forEach((image, index) => {
                        if (image.file instanceof File) {
                            ...
                            const inputRef = this.$refs[this.$.uid + '_imageInput_' + index];

                            if (inputRef && inputRef[0]) {
                                inputRef[0].files = dataTransfer.files;
                            }
                        } else {
                            image.is_new = 0;
                            image.url = this.resolveImageUrl(image.path);
                        }
                    });
                },
                ...
                resolveImageUrl(path) {
                    ...
                    return '{{ Storage::url('') }}' + path.replace(/^\/?/, '');
                },
