...
>
    ...
    <template v-else>
        ...
        <div class="box-shadow rounded" v-if="report.statistics.length">
            <!-- Single Product -->
            <div class="relative" v-for="product in report.statistics" :key="product.id">
                <div class="row grid grid-cols-2 gap-y-6 border-b bg-white p-4 transition-all hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:hover:bg-gray-950 max-sm:grid-cols-[1fr_auto]">
                    <div class="flex gap-2.5">
                        ...
                        <div class="flex flex-col gap-1.5">
                            ...
                            <p class="text-gray-600 dark:text-gray-300">
                                @{{ skuLabel.replace(':replace', product.sku) }}
                            </p>
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between gap-1.5">
                        <div class="flex flex-col gap-1.5">
                            ...
                            <!-- Total Product Stock -->
                            <p :class="product.total_qty > outOfStockThreshold ? 'text-emerald-500' : 'text-red-500'">
                                @{{ totalStockLabel.replace(':replace', product.total_qty) }}
                            </p>
                        </div>

                        <!-- View More Icon -->
                        <a :href="editUrl.replace(':replace', product.id)">
        ...
        <div class="box-shadow rounded" v-else>
...
app.component('v-dashboard-stock-threshold-products', {
    ...
    data() {
        return {
            ...
            // Blade -> Vue variables
            outOfStockThreshold: {{ core()->getConfigData('catalog.inventory.stock_options.out_of_stock_threshold') }},
            skuLabel: `{{ __('admin::app.dashboard.index.sku', ['sku' => ':replace']) }}`,
            totalStockLabel: `{{ __('admin::app.dashboard.index.total-stock', ['total_stock' => ':replace']) }}`,
            editUrl: `{{ route('admin.catalog.products.edit', ':replace') }}`
        }
    },
    ...
    methods: {
        getStats(filters) {
            ...
            params.type = 'stock-threshold-products';
...
getImageUrl(url) {
    ...
    if (!url) {
        console.log('=> No URL provided, return empty string');
        return '';
    }

    // Nếu có cả /storage/ và chứa http/https phía sau nó
    const storageIndex = url.indexOf('/storage/');
    if (storageIndex > -1) {
        const httpAfterStorage = url.indexOf('http', storageIndex);
        if (httpAfterStorage > -1) {
            const finalUrl = url.substring(httpAfterStorage);
            console.log('=> External URL after /storage/ detected:', finalUrl);
            return finalUrl;
        }
    }

    // Nếu URL chứa http/https bình thường (1 lần duy nhất)
    if (/^https?:\/\//.test(url)) {
        console.log('=> Single http(s) found, keep as is:', url);
        return url;
    }
    ...
    if (url.startsWith('/')) {
        ...
        console.log('=> Internal absolute path:', fixedUrl);
        return fixedUrl;
    }

    // Nếu là đường dẫn tương đối
    const finalUrl = window.location.origin + '/' + url.replace(/^\/+/, '');
    console.log('=> Default relative path:', finalUrl);
    return finalUrl;
}
