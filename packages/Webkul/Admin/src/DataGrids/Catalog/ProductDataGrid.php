...
use Illuminate\Support\Str; // THÊM DÒNG NÀY VÀO
        ...
        $this->addColumn([
            ...
            'closure'    => function ($row) {
                if (! $row->base_image) {
                 ...
                 // Kiểm tra nếu base_image đã là một URL đầy đủ
                if (Str::startsWith($row->base_image, ['http://', 'https://'])) {
                    return $row->base_image; // Trả về URL đầy đủ trực tiếp
                }

                // Nếu không, coi đó là đường dẫn tương đối và sử dụng Storage::url()
                return Storage::url($row->base_image);
                // return asset('storage/'.$row->base_image);
            },
        ]);
